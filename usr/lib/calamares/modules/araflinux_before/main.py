...
def move_mkinitcpio_preset():
    target_root = libcalamares.globalstorage.value("rootMountPoint")
    src = os.path.join(target_root, "etc/mkinitcpio.d/araflinux")
    dst = os.path.join(target_root, "etc/mkinitcpio.d/linux.preset")

    libcalamares.utils.debug("-> Moving araflinux preset to linux.preset in target...")
    try:
        os.replace(src, dst)
...
def run():
    ...
    libcalamares.utils.debug("Start araflinux_before")
    libcalamares.utils.debug("#################################\n")
    ...
    for step_func in [
        ...
        if error:
            return error
    libcalamares.utils.debug("#################################")
    libcalamares.utils.debug("End araflinux_before")
    libcalamares.utils.debug("#################################\n")
    return None
